# gittish-core-protocol-organization-create

This package defines the service interface that is responsible for creating new
organizations.

The service supports the following use cases

* create a new, uniquely named, organization

When using this service, one must first make sure that the requested name does
not already exist. To accomplish that, one must check the requested name via
the organization name lookup service prior to calling this service.

The standard course of action for this service does not include the creation of
data that is supplemental to the organization.

In order to fully create a new organization, one must use the organization
creation workflow service, which in turn will call upon this service.
...
```
cd <WORKSPACE_ROOT>/gittish-core-protocol-organization-create
./configure
```
